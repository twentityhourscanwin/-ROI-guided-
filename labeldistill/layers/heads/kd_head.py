...
class KDHead(CenterHead):
    ...
    def response_loss(self, targets, preds_dicts, teacher_dicts, **kwargs):
        ...
        stduent_heatmap, teacher_heatmap = [], []
        for task_id, (preds_dict, teacher_dict) in enumerate(zip(preds_dicts, teacher_dicts)):
            ...
            loss_heatmap_distill = self.loss_cls(preds_dict[0]['heatmap'].type(torch.float32),
                                                 teacher_dict[0]['heatmap'].type(torch.float32) * heatmaps[task_id],
                                                 avg_factor=cls_avg_factor)

            # stduent_heatmap.append(preds_dict[0]['heatmap'].type(torch.float32))
            # teacher_heatmap.append(teacher_dict[0]['heatmap'].type(torch.float32))
