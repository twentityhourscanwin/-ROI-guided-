#LabelDistill/labeldistill/exps/nuscenes/labeldistill/LidarDistill_r101_128x128_e24_4keyfp.py
...
class LabelDistillModel(BaseLabelDistillModel):
    ...
    def __init__(self, **kwargs):
        ...
        self.fp_distill_start_epoch = 26  # 开始计算的epoch
        self.fp_distill_warmup_epochs = 2  # 权重从0增加到目标值的epoch数
        self.fp_distill_max_weight = 0.2  # 最终权重
    ...
    def on_train_start(self):
        ...
        if is_ddp_wrapped:
            ...
            # 2. **关键修复点：** 对 DDP 实例调用 _set_static_graph()
            if self.backbone_conf.get('img_backbone_conf', {}).get('with_cp', False):
                if hasattr(self.trainer.model, '_set_static_graph'):
                    ...
                    # 关键修改：直接对 self.trainer.model (DDP实例) 调用方法
    ...
    def training_step(self, batch):  
        ...
        depth_loss = self.get_depth_loss(depth_labels.cuda(), depth_preds,1.5)
        ...
        if fp_weight > 0:
            bev_roi_mask_fp = self.generate_bev_mask_fp(filtered_results, B)
        ...
        total_loss = detection_loss + depth_loss + lidar_distill_loss + response_loss + fp_distill_loss
    ...
    def configure_optimizers(self):
        ...
        # ===== Warmup + MultiStep配置 =====
        warmup_iters = 200          # 前200步warmup
        warmup_ratio = 0.001        # 从千分之一开始
        milestones = [19, 23]       # 在第19和21个epoch衰减
        gamma = 0.1                 # 每次衰减到0.1倍
        ...
        def lr_lambda(current_step):
            ...
            Phase 1 (步数0-199): Warmup阶段
                LR从 base_lr * 0.001 线性增长到 base_lr * 1.0

            Phase 2 (步数200+): MultiStep衰减
                - Epoch 0-18:  LR = base_lr
                - Epoch 19-20: LR = base_lr * 0.1
                - Epoch 21+:   LR = base_lr * 0.01
            """
        ...
        # ⚠️ 重要：返回配置字典，指定调度器在每步更新
        return {
            'optimizer': optimizer,
            'lr_scheduler': {
                'scheduler': scheduler,
                'interval': 'step',  # 每步更新（而非每epoch）
                'frequency': 1
            }
        }
...
if __name__ == '__main__':
    run_cli(LabelDistillModel,
            'LidarDistill_r101_128x128_e24_4keyfp_alltry',
            extra_trainer_config_args={'epochs': 21},
            use_ema=True)